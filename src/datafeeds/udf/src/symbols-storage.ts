				...
				.then((response: ExchangeDataResponse) => {
					try {
						...
						if (error instanceof Error) {
							// Objek error adalah instance dari Error, Anda dapat melemparkannya ke dalam reject
							reject(error);
						} else {
							// Jika 'error' bukan objek kesalahan, Anda dapat membuat Error baru
							reject(new Error(`SymbolsStorage: API error when processing exchange ${exchange} symbol: ${String(error)}`));
						}
						return;
					}
	...
	private _onExchangeDataReceived(exchange: string, data: ExchangeDataResponse): void {
		...
		try {
			...
			if (error instanceof Error) {
				...
				throw new Error(`SymbolsStorage: API error when processing exchange ${exchange} symbol #${symbolIndex} (${data.symbol[symbolIndex]}): Unknown error occurred`);
			}
		}
	}
}
